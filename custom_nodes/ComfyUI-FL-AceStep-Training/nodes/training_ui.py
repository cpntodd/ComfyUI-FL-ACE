...
class FL_AceStep_Train:
    ...
    def train(self, model, config, tensor_dir, lora_name="my_lora", resume_from="", unique_id=None):
        ...
        # Make a private training copy; ComfyUI patcher clones share their model.
        ...
        try:
            ...
            training_model.model.diffusion_model = dit
            if device.type == "xpu":
                dit.to(device)
            decoder = dit.decoder
                ...
                for epoch in range(start_epoch, training_config.max_epochs):
                    ...
                    for batch in dataloader:
                        ...
                        try:
                            ...
                            logger.exception("ACE-Step training forward pass failed")
                            raise
