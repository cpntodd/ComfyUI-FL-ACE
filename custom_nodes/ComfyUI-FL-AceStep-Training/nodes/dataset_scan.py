...
class AudioSample:
    ...
    lyrics_variants: Dict[str, str] = field(default_factory=dict)
...
class ACEStepDatasetBuilder:
    ...
    def _create_sample(
        ...
        # Load lyrics from a same-stem .txt file or shared song-folder markdown.
        lyrics = ""
        ...
        lyrics_variants = self._load_folder_lyrics(audio_path.parent)
        if not lyrics:
            lyrics = lyrics_variants.get("mk", "")
        ...
        return AudioSample(
            ...
            raw_lyrics=lyrics,
            lyrics_variants=lyrics_variants,
            bpm=bpm,
            keyscale=keyscale,
            duration=duration,
    ...
    def _load_folder_lyrics(self, folder: Path) -> Dict[str, str]:
        variants = {}
        for language, filename in (
            ...
            try:
                ...
                logger.warning(f"Could not load lyrics from {path}: {e}")
                continue
            if text:
                variants[language] = text
        return variants
            ...
            "samples": [
                {
                    ...
                    "genre": s.genre,
                    "lyrics": s.lyrics,
                    "lyrics_variants": s.lyrics_variants,
                    "bpm": s.bpm,
                    "keyscale": s.keyscale,
