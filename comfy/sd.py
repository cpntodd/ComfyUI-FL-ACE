...
def load_state_dict_guess_config(sd, output_vae=True, output_clip=True, output_clipvision=False, embedding_directory=None, output_model=True, model_options={}, te_model_options={}, metadata=None, disable_dynamic=False):
    ...
    if output_model:
        if inital_load_device != torch.device("cpu"):
            ...
            # The ACE-Step checkpoint can exceed an Intel XPU's free VRAM during
            # the initial full-load copy even when managed offload can run it.
            force_full_load = not model_management.is_device_xpu(inital_load_device)
            model_management.load_models_gpu([model_patcher], force_full_load=force_full_load)
